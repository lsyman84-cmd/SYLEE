...
def cached_load_db(file_bytes: bytes | None, default_path: str) -> pd.DataFrame:
    if file_bytes is not None:
        with tempfile.NamedTemporaryFile(suffix=".xlsx", delete=False) as tmp:
            tmp.write(file_bytes)
            tmp_path = tmp.name
        try:
            return load_equipment_db(tmp_path)
        finally:
            Path(tmp_path).unlink(missing_ok=True)
    return load_equipment_db(default_path)
...
def main():
    ...
    with st.sidebar:
        ...
        default_db = PROJECT_ROOT / "Mechanical Equipment Database.xlsx"
        uploaded = st.file_uploader(
            "Excel DB 업로드 (없으면 기본 파일 사용)",
            type=["xlsx"],
            help="기본 파일: Mechanical Equipment Database.xlsx",
        )
        ...
        if uploaded:
            ...
            file_bytes = None
            st.info(f"기본 파일 사용: {default_db.name}")
        else:
            st.error("DB 파일을 찾을 수 없습니다. 파일을 업로드해 주세요.")
            st.stop()

        with st.spinner("DB 로딩 중..."):
            db = cached_load_db(file_bytes, str(default_db))
