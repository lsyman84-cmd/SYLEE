...
_FONT_CANDIDATES = [
    # 레포 내 번들 폰트 (최우선 — 모든 환경에서 동작)
    (str(_PROJECT_ROOT / "fonts" / "NanumGothic.ttf"),
     str(_PROJECT_ROOT / "fonts" / "NanumGothicBold.ttf"),
     "NanumGothic", "NanumGothicBold"),
    # Windows (로컬 개발 fallback)
    (r"C:\Windows\Fonts\malgun.ttf", r"C:\Windows\Fonts\malgunbd.ttf",
     "Malgun", "MalgunBold"),
    # Linux 시스템 폰트 fallback
    ("/usr/share/fonts/truetype/nanum/NanumGothic.ttf",
     "/usr/share/fonts/truetype/nanum/NanumGothicBold.ttf",
     "NanumGothic", "NanumGothicBold"),
]

for _reg_path, _bold_path, _reg_name, _bold_name in _FONT_CANDIDATES:
    try:
        _reg_registered = False
        _bold_registered = False
        if Path(_reg_path).exists():
            pdfmetrics.registerFont(TTFont(_reg_name, _reg_path))
            _KR = _reg_name
            _reg_registered = True
        if Path(_bold_path).exists():
            pdfmetrics.registerFont(TTFont(_bold_name, _bold_path))
            _KRB = _bold_name
            _bold_registered = True
        if _reg_registered and _bold_registered:
            registerFontFamily(_KR, normal=_KR, bold=_KRB, italic=_KR, boldItalic=_KRB)
            break
    except Exception:
        pass
